  ...
  public:
    ...
    void update(float dt) override {
      ...
      for (auto entity : entities_) {
        auto& grid = ctx->getComponent<Grid>(entity);
        ...
        // Vertices
        glBindBuffer(GL_ARRAY_BUFFER, glVbo_);
        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec4) * grid.vertices.size(), grid.vertices.data());
        // Indices
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, glEbo_);
        glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, sizeof(unsigned int) * grid.indices.size(), grid.indices.data());
